...
- makefile 用 `-MMD -MP` 自动追踪头文件依赖（`build/*.d`，`make clean` 清理）；改头文件无需手动 `make clean`。
...
- 资源在 `res/`：`app.rc`（UTF-8 编码，`#pragma code_page(65001)`，含中文对话框文案，勿改为系统默认编码）、`resource.h`（资源 ID）、`app.manifest`（以资源类型 24 嵌入，DPI/per-monitor 声明在此）。改 .rc/.h 触发 windres 重编。`res/web/` 为 Markdown 预览的离线资源（marked / KaTeX / 字体），以 `RCDATA` 内嵌进 exe。
- 版本号硬编码在 `src/TrayIcon.cpp:98`（托盘"关于"文案）。历史发布即提交 "Bump version to vX.YZ"，发版需同步改此处。

## 架构

入口 `src/main.cpp`（114 行）：`wWinMain` 里以全局指针装配 `g_settings` / `g_tray` / `g_hotkey` / `g_session`，消息循环驱动。核心流程：
...
HotkeyManager(全局热键) → ScreenshotSession(会话协调/截屏冻结)
  ...
  → ResultWindow(展示结果) → MarkdownPreview(浏览器离线预览)
```

- `AiService`：OpenAI 兼容 HTTP（winhttp），BaseUrl/ApiKey/模型可配。PNG→base64 编码在 `gdiutil::HBitmapToBase64Png`。
...
- GDI+ 非线程安全：后台 AI 线程用 GDI+ 编码 PNG，须按线程 `GdiplusStartup`/`Shutdown`（见 `gdiutil::HBitmapToBase64Png` 内守卫，勿删——删了会重现堆损坏 0xc0000374）。
